...
export type RegistrationFeeState = { amount: bigint; treasury: `0x${string}` | null };
export type RegistrationFeeDetails = RegistrationFeeState & { displayUsd: string };
...
export class RegistrationFeeService {
  ...
  async getState(): Promise<RegistrationFeeState> {
    if (this.config.mode !== "live" || !this.config.feeManager || !this.config.rpcUrl) {
      return { amount: 0n, treasury: null };
    }
    const providerService = ProviderService.fromConfig(this.config);
    return new FeeManagerService(providerService, this.config.feeManager).getRegistrationFeeState();
  }
