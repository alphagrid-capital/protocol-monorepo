...
JSON request/response mode (`enableJsonResponse`). Not a persistent SSE stream for tool calls.

#### Session ID and SSE

`GET /mcp` returns a short-lived SSE acknowledgment. Tool execution uses `POST` with JSON bodies. Sessions are per-isolate memory.
