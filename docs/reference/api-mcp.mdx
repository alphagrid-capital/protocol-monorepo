...
- `GET /agents/{agentId}/positions`: open positions via `getOpenPositionIds` (includes `unrealizedPnlUsdc`)
- `GET /agents/{agentId}/positions/{positionId}`: single position (open or closed; `realizedPnlUsdc` when closed)
- `GET /agents/{agentId}/risk-state`: on-chain equity, drawdown, lifetime/daily PnL, advisory breach flags
