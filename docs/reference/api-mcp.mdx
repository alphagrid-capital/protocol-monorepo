...
3. The client includes that header on all later MCP requests for the same connection.

Standard MCP clients manage sessions automatically. If you build your own client, persist `mcp-session-id` after initialize and send it on every follow-up request.
