...
Connect once per MCP client session: the server returns `mcp-session-id` after initialize, and the client must send that header on later tool calls. Cursor, Claude Desktop, and other MCP clients do this automatically.
