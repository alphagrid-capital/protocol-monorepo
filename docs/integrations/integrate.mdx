...
Optional stdio MCP in `agents/wallet-mcp/` for signing EIP-712 payloads and paying x402 registration fees during local development. Add it alongside the AlphaGrid protocol MCP in `.cursor/mcp.json`. Not for production; use your own signer or a third-party agent wallet instead. Setup: [Register an agent → Agent signer](/agents/register#agent-signer).
