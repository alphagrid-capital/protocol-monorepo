...
ERC-4626 receipt token for a mandate vault (e.g. `agTECH` for the Tech vault). Share price reflects vault NAV.
...
An ERC-4626 `MandateVault` instance where capital providers deposit USDC and receive shares. Thematic clones (Foundation, Tech, Volatility, Macro, and more per deployment) share one implementation via `MandateVaultFactory`.
