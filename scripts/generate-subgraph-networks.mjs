...
const LENS_FALLBACK_START_BLOCK_CONTRACT = 'TradeRouter'
...
for (const [chainIdStr, networkName] of Object.entries(GRAPH_NETWORKS)) {
  ...
  for (const contract of CONTRACTS) {
    ...
    let startBlock = blocks[contract]
    if (
      contract === 'TradeRouterLens' &&
      startBlock === undefined &&
      blocks[LENS_FALLBACK_START_BLOCK_CONTRACT] !== undefined
    ) {
      startBlock = blocks[LENS_FALLBACK_START_BLOCK_CONTRACT]
    }
...
const contractsTsLines = [
  '// Generated by scripts/generate-subgraph-networks.mjs — do not edit.',
  "import { Address, dataSource } from '@graphprotocol/graph-ts'",
  '',
]

for (const [networkName, contracts] of Object.entries(networks)) {
  const lens = contracts.TradeRouterLens?.address
  const router = contracts.TradeRouter?.address
  if (!lens || !router) {
    continue
  }
  const prefix = networkName.replace(/-/g, '_').toUpperCase()
  contractsTsLines.push(
    `const ${prefix}_LENS = Address.fromString('${lens}')`
  )
  contractsTsLines.push(
    `const ${prefix}_ROUTER = Address.fromString('${router}')`
  )
}
...
const contractsTsPath = join(root, 'subgraph/src/lib/network-contracts.ts')
writeFileSync(contractsTsPath, contractsTsLines.join('\n'))
console.log(`Wrote ${contractsTsPath}`)
